...
# Phase 8.2-B2 — Interactive Quiz
from web.components.quiz_panel import render_quiz_panel, get_quiz_evaluation_dict

# Phase 8.3-B1 — AI Teaching Material
from web.components.material_panel import render_material_panel
...
def main():
    ...
    def init_workflow(provider_mode: str) -> A3Workflow:
        """A3Workflow instance per provider mode (auto | mock | spark | rule)."""
        provider = None
        if provider_mode == "auto":
            # Read user-configured provider from llm.json
            provider = create_provider() or create_provider("mock")
        elif provider_mode in ("mock", "spark"):
    ...
    with tab1:
        ...
        user_cfg = load_llm_config()
        configured_label = (
            f"🧠 已配置 LLM ({user_cfg.provider_label})"
            if user_cfg.is_configured
            else "🧠 已配置 LLM (未配置)"
        )
        PROVIDER_OPTIONS = {
            configured_label: "auto",
            "🤖 Mock (演示 LLM)": "mock",
            "🚀 Xunfei Spark": "spark",
            "⚙️ Rule (纯规则)": "rule",
        }
        # Default to user-configured provider if available, else mock
        default_idx = 0  # "auto" is first
        ...
        with col_mode:
            if workflow.llm_provider is not None:
                ...
                _name = type(workflow.llm_provider).__name__
                st.caption(f"当前: 🤖 {_name} ({_model})")
            else:
                st.caption("当前: ⚙️ Rule Mode")
        ...
        if st.session_state.pipeline_run:
            ...
            display_order = ["ProfileAgent", "PlannerAgent", "ContentGeneratorAgent",
                              "ResourceAgent", "ReviewAgent", "ReflectionAgent", "Memory"]
            ...
            # ── Phase 8.2-B2: Interactive Quiz ──
            quiz_eval = render_quiz_panel(
                provider=workflow.llm_provider,
                topic=text,
            )
            if quiz_eval:
                # Inject quiz weak/strong areas into workflow result
                # so Phase 8.2-B1 mastery updates can use them
                wr = st.session_state.workflow_result
                if wr and "evaluation" in wr:
                    wr["evaluation"].update(quiz_eval)
                    st.session_state.workflow_result = wr

            # ── Phase 8.3-B1: AI Teaching Material ──
            render_material_panel(st.session_state.workflow_result.get("content"))
    ...
    with tab3:
        ...
        if not result_dict:
            ...
            # ── Phase 8.3-B1: AI Teaching Material in 学习空间 ──
            render_material_panel(result_dict.get("content"))
